...
% --- Case 1: Start at (1, -1) ---
fprintf('Processing Levenberg (Fixed Step 0.8) for (1, -1)...\n');
[~, k1, x1, y1, g1, msg1] = levenberg_fixed(f, [-1, -1], tol, x, y);
visualize_results(k1, x1, y1, f, g1, msg1);

% --- Case 2: Start at (-1, 1) ---
fprintf('Processing Levenberg (Fixed Step 0.8) for (-1, 1)...\n');
[~, k2, x2, y2, g2, msg2] = levenberg_fixed(f, [1, 1], tol, x, y);
visualize_results(k2, x2, y2, f, g2, msg2);

% --- Case 3: Start at (0, 0) ---
fprintf('Processing Levenberg (Fixed Step 0.8) for (0, 0)...\n');
[~, k3, x3, y3, g3, msg3] = levenberg_fixed(f, [0, 0], tol, x, y);
...
function visualize_results(k, x_hist, y_hist, f_sym, g_array, msg)
    ...
    xlabel('x'); ylabel('y');
    title({sprintf('Levenberg (Fixed Step 0.8) [%s, %s]', x0_str, y0_str), ['Result: ' msg]});
    grid on; axis equal;
    xlim([-2.5 2.5]); ylim([-2.5 2.5]);
    %saveas(gcf, ['contour_levenberg_fixed_x0_', x0_str, '_y0_', y0_str, '.png']);
    ...
    if ~isempty(g_array)
        ...
        title('Step Size (Fixed Constant)');
        ylim([0 1]); % Set limits to see the line clearly
        grid on;
...
function [x_curr, k, x_hist, y_hist, g_hist, msg] = levenberg_fixed(f_sym, x_0, tol, sym_x, sym_y)
    
    % Create handles
    f_func = matlabFunction(f_sym); % Not strictly needed for logic, but good for debugging
    ...
    while k < max_iter
        ...
        % Shift eigenvalues by adding Identity matrix
        if min_eig <= 0
            delta = 0.1; 
