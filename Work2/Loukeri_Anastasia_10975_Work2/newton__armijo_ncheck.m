...
% --- Case 1: Start at (1, -1) ---
fprintf('Processing Newton (Armijo, No Check) for (-1, -1)...\n');
[~, k1, x1, y1, g1, msg1] = newton_armijo_nocheck(f, [-1, -1], tol, x, y);
visualize_results(k1, x1, y1, f, g1, msg1);

% --- Case 2: Start at (-1, 1) ---
fprintf('Processing Newton (Armijo, No Check) for (1, 1)...\n');
[~, k2, x2, y2, g2, msg2] = newton_armijo_nocheck(f, [1, 1], tol, x, y);
visualize_results(k2, x2, y2, f, g2, msg2);

% --- Case 3: Start at (0, 0) ---
fprintf('Processing Newton (Armijo, No Check) for (0, 0)...\n');
[~, k3, x3, y3, g3, msg3] = newton_armijo_nocheck(f, [0, 0], tol, x, y);
visualize_results(k3, x3, y3, f, g3, msg3);

% --- Case 4: Start at (-1.2, -0.5) ---
fprintf('Processing Newton (Armijo, No Check) for (-1.2, -0.5)...\n');
[~, k4, x4, y4, g4, msg4] = newton_armijo_nocheck(f, [-1.2, -0.5], tol, x, y);
...
function visualize_results(k, x_hist, y_hist, f_sym, g_array, msg)
    ...
    xlabel('x'); ylabel('y');
    title({sprintf('Newton (Armijo, No Check) [%s, %s]', x0_str, y0_str), ['Result: ' msg]});
    grid on; axis equal;
...
% NEWTON (NO HESSIAN CHECK) + ARMIJO
function [x_curr, k, x_hist, y_hist, g_hist, msg] = newton_armijo_nocheck(f_sym, x_0, tol, sym_x, sym_y)
    ...
    while k < max_iter
        ...
        % --- NO POSITIVE DEFINITE CHECK ---
        % Only Singularity Check to prevent crash
        ...
        % Note: If dk is an ascent direction (because H was negative definite),
        % Armijo will likely reduce gamma to near 0.
...
function gamma = armijo_search(f_func, grad_val, dk, x_k, g_0, sigma, beta)
    ...
    % If failed to find a step, return very small step
