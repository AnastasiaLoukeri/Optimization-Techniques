...
for i = 1:3 % Loop μέσω των 3 συναρτήσεων
    ...
    for j = 1:length(l_values) % Loop μέσω των διαφόρων τιμών του l
        ...
        % 1. Εκτέλεση αλγορίθμου και λήψη ιστορικού
        [a_hist, b_hist, k_hist] = bisection_search_history(f, a, b, eps_const, l);
    ...
    title(sprintf('Convergence of Interval [a_k, b_k] vs k for %s (\\epsilon = %.3f)', function_title, eps_const));
    xlabel('Iterations k');
