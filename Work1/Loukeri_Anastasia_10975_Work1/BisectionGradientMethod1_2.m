clc; clear; close all;
f_data = struct(...
    'f', { @(x) 5.^x*log(5) - 2*sin(x).*(cos(x) - 2), @(x) 2*x + cos(x + 3).*exp(x - 5) + sin(x + 3).*exp(x - 5) - 2, @(x) -3.*exp(-3.*x) + 2.*cos(x-2).*(2 - sin(x-2))}, ...
    'a', {-1, -1, -1}, ...
    'b', {3, 3, 3}, ...
    'L0', {5, 6, 9}, ...
...
for i = 1:3 % Loop μέσω των 3 συναρτήσεων
    ...
    for j = 1:length(l_values) % Loop μέσω των διαφόρων τιμών του l
        ...
        % 1. Ληψη ιστορικού
        [a_hist, b_hist, k_hist] = bisection_gradient_history(f, a, b, l);
        ...
        % 2. Σχεδίαση (k, ak)
        plot(k_hist, a_hist, [l_colors{j}, l_styles{j}], 'LineWidth', 1.9, ...
             'DisplayName', sprintf('a_k, l=%.3f (K=%d)', l, k_hist(end)));
        
        % 3. Σχεδίαση (k, bk)
        plot(k_hist, b_hist, [l_colors{j}, l_styles{j}], 'LineWidth', 1.5, ...
             'DisplayName', sprintf('b_k, l=%.3f (K=%d)', l, k_hist(end)));
    end
    
    % Τίτλοι και Επεξηγήσεις
    title(sprintf('Convergence of Interval [a_k, b_k] vs k for %s ', function_title));
    xlabel('Iterations k');
