...
a = -1;           % Κάτω όριο
b = 3;           % Άνω όριο
epsilon = 0.001;  % Σταθερά διαχωρισμού ε
l_values = 0.003:0.0002:0.1; % Επιθυμητή ακρίβεια τελικού μήκους 
for i = 1:length(l_values)
    current_l = l_values(i);
    % Εκτέλεση και καταγραφή N
    [~, ~, N1_results(i)] = bisection_search(f1, a, b, epsilon, current_l);
    [~, ~, N2_results(i)] = bisection_search(f2, a, b, epsilon, current_l);
    [~, ~, N3_results(i)] = bisection_search(f3, a, b, epsilon, current_l);
...
title('Bisection Search: N(l)', 'FontSize', 17);
legu1 = legend('f1');
